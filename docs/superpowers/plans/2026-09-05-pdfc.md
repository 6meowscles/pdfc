...
def test_trailing_slash_target_is_treated_as_a_directory(tmp_path):
    paths = output_paths(Path(f"{tmp_path}/out/"), "scan", 2, "png")
...
def _is_directory_target(target: Path, raw: str) -> bool:
    return target.is_dir() or raw.endswith("/")
...
def output_paths(target: Path, stem: str, count: int, extension: str) -> list[Path]:
    ...
    if _is_directory_target(target, raw):
...
def build_plan(
    ...
    target_is_dir = _is_directory_target(target, str(target))
...
Click's `CliRunner` invokes `main` directly, so the error mapping must also live in the group. Add this immediately after the `PdfcGroup` class and change `main`'s decorator to `@click.group(cls=PdfcGroup, invoke_without_command=False)`:
...
def test_md_to_html_renders_headings_and_tables(sample_md, tmp_path):
